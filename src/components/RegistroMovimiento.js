...
const RegistroMovimiento = ({
  ...
  const addOrEditMovimiento = (e) => {
    e.preventDefault();
    if (permiteAgregarEditarMovimiento()) {
  ...
  function editarMovimiento() {
    const newListMovimientos = listMovimientos.map((item) => {
          ...
          <form onSubmit={addOrEditMovimiento}>
            <div className="row">
              <div className="col-lg-12 mb-3">
                <label className='form-label' htmlFor='tipoMovimiento'>Tipo Movimiento:</label>
                <select id="tipoMovimiento" name="tipoMovimiento" className="form-select" aria-label="Default select example" value={values.tipoMovimiento} onChange={({ target }) => handlerChange(target.name, target.value)}>
            ...
            <div className="row">
              <div className="col-lg-12 text-center">
                <button type='button' className="btn btn-secundary" onClick={() => cancelar}>Cancelar</button>
